...
func init() {
	os.Setenv("APP_NAME", "auto_booking")
	log.InitLogger(false)
	os.Setenv("TZ", "Asia/Ho_Chi_Minh")
	fmt.Println("AutoBooking Production - "+time.Now().String())
	log.Info("AutoBooking Production - "+time.Now().String())
}

func main()  {

	sql := &db.Sql{
		Host:     "10.36.40.11", //localhost,host.docker.internal
		Port:     5432,
		UserName: "nim",
		Password: "nim2021",
		DbName:   "appbooking",
	...
	seConNim := &automate_impl.Selenium{
		...
		ConnectServer: "http://10.36.40.11:4444/wd/hub",
	}
